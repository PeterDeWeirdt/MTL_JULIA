function EstimateMultipleFitsTRN(geneExprMats,tfaMats,lambdaBias,tfaOpt,...
    method,lambdaBMin,lambdaBMax,totLogLambdaBSteps,lambdaSMin,...
    lambdaSMax,totLogLambdaSSteps,fitOutMats,leaveOutSampleLists, parallel, ntasks)
%% Goal: Estimate fit (ebic, aic or bic) for dirty multi-task lasso for given input prior, prior 
% reinforcement, TFA methods, and fit method
%% References:
% Miraldi et al. "Leveraging chromatin accessibility data for 
%   transcriptional regulatory network inference in T Helper 17 Cells"
% Zhou et al. (2011) "Malsar: Multi-task learning via structural regularzation."
...
%   Informatics, Cincinnati Children's Hospital
%% INPUTS:
% geneExprMats -- a cell of .mat files, each containing gene expression data, gene lists
%   (target genes, potential regulators,...), e.g., as generated by
%   importGeneExpGeneLists.m
% tfaMats -- a cell of .mat files containing the prior of TF-gene interactions as
...
% lambdaBMin -- initial guess for lambda lower bound for block penalty, [0, infinity), note glmnet
%   is very slow for lambda < .01
% lambdaBMax -- initial guess for lambda upper bound for block penalty, [0, infinity)
% totLogLambdaBSteps -- number of steps per log10 lambda range in
%   lambda for block pentalty 
% lambdaSMin, lambdaSMax, totLogLambdaSSteps --  " " for sparse penalty
% FitOutMats -- full file name and paths for output .mat files, will also
...
% parallel -- whether to parallelize fits 
% ntasks -- number of tasks
%% OUTPUTS:
% OutMats -- contains network- and gene-level fits,
%   lambdaRange
% OutMat.fig's + .pdf's -- showing network- and gene-level fits
...
predictorMats = cell(1, ntasks); responseMats = predictorMats; priorWeightsMats = predictorMats;
for i = 1:ntasks
    load(geneExprMats{i})
    ...
    leaveOutSampleList = leaveOutSampleLists{i};
    ...
    predictorMats{i} = predictorMat; responseMats{i} = responseMat;
    priorWeightsMats{i} = priorWeightsMat;
end

%% get fit
disp('Getting model fit')
lamBLog10step = 1/totLogLambdaBSteps;
logLamBRange =  log10(lambdaBMin):lamBLog10step:log10(lambdaBMax);
lambdaBRange = [10.^logLamBRange]';
inlambdaBRange = fliplr(lambdaBRange');
lamSLog10step = 1/totLogLambdaSSteps;
logLamSRange =  log10(lambdaSMin):lamSLog10step:log10(lambdaSMax);
lambdaSRange = [10.^logLamSRange]';
inlambdaSRange = fliplr(lambdaSRange');
...
[lamB_lamS, netFits] = getMultipleFits(predictorMats, responseMats, priorWeightsMats,...
    inlambdaBRange, inlambdaSRange, method, parallel, ntasks, opts);
