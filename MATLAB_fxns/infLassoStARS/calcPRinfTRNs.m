...
if gsRegsFile
    ...
    gsRegInds = find(ismember(gsRegsTmp,netPotRegs)); % consider all TFs
end
if targGeneFile
    gsTargInds = find(ismember(gsTargsTmp,potTargGenes));
else
    gsTargInds = find(ismember(gsTargsTmp,netTargGenes)); % consider all target genes
end
